...
#
# See vllm_ascend/worker/v2/spec_decode/doc/resample_kernel.md for the operator
# spec.
#
...
#
# The kernel calls the `_npu_gumbel_block_argmax` device function, whose Gumbel
# noise comes from Triton's philox and has no PyTorch equivalent.
# `_gumbel_noise_probe_kernel` below replays the *same* stream so the reference
# can consume it; what these tests then check is everything the kernel itself
# owns -- the three residual branches, the early return, and the block tiling.
# The device function is covered on its own in
# `test_npu_gumbel_block_argmax.py`; `_gumbel_noise_probe_kernel` is duplicated
# there and the two copies must stay byte-identical to the RNG block of
...
def test_resample_greedy_bonus_is_plain_argmax():
    ...
    bonus_rows = torch.tensor(
        [batch["cu"][r + 1] - 1 for r in range(num_reqs)], dtype=torch.long, device=DEVICE
    )
    ref_val, ref_idx = _ref_block_argmax(
        batch["target_logits"][bonus_rows], vocab_size, RESAMPLE_BLOCK_SIZE
    )
...
def test_resample_one_hot_draft_excludes_rejected_token():
    ...
    _assert_block_argmax_close(
        argmax, local_max, ref_idx, ref_val, residual + noise, RESAMPLE_BLOCK_SIZE
    )
...
def test_resample_draft_logits_residual_matches_reference():
    ...
    draft_logits = torch.randn(
        max_num_reqs, num_spec_steps, vocab_size, dtype=torch.float32, device=DEVICE
    )
    ...
    draft_lse = torch.stack(
        [
            torch.logsumexp(draft_logits[int(batch["rows"][r]), int(rejected_step[r])], dim=0)
            for r in range(num_reqs)
        ]
    ).contiguous()
...
def test_resample_mixed_batch_keeps_requests_independent():
    ...
    for r in range(num_reqs):
        ...
        ref_val, ref_idx = _ref_block_argmax(
            residual.unsqueeze(0), vocab_size, RESAMPLE_BLOCK_SIZE, noise=noise
        )
...
def test_rejection_sample_greedy_end_to_end():
    ...
    expanded_local_pos = (
        torch.arange(num_logits_per_req, dtype=torch.int32).repeat(num_reqs).to(DEVICE)
    )
